...
import { RACES, FACTIONS, standingLabel } from "@/game/gc/races";
import { SITE_BY_ID } from "@/game/gc/sites";
import {
  CRAFT_BY_ID,
  CORRIDOR_BY_ID,
  CORRIDORS,
    ...
    <figure className="mt-2 border border-rule bg-paper3/40">
      <div className="relative aspect-[8/5] w-full overflow-hidden">
        {src ? (
          <div
            role="img"
            aria-label={`Fotografía tramada de ${name}`}
      ...
      <figcaption className="mono truncate border-t border-rule px-1.5 py-0.5 text-[8px] uppercase tracking-wider text-ink3">
        {credit ? `${credit.licence} · commons` : "sin fotografía en el archivo"}
      </figcaption>
    </figure>
  );
}
    ...
    <div className="paper-card p-2.5">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="mono shrink-0 text-right text-[10px]">
          <div>sospecha {Math.round(suspicion)}</div>
      ...
      <div className="mt-2.5 flex flex-wrap gap-1.5">
        {!linkFrom && canSource && (
          ...
          >
            {idleCraft.length ? "Conectar una ruta desde aquí" : "Sin naves libres"}
          </button>
        )}
      ...
      <div className="mono mt-2 grid grid-cols-3 gap-1.5 text-[10px]">
        <div className="border border-rule px-1.5 py-1">
          <div className="text-ink3">RENDIMIENTO</div>
          <div className="text-[13px]">{site.yield || "—"}</div>
        </div>
        <div className="border border-rule px-1.5 py-1">
          <div className="text-ink3">ATRACTIVO</div>
          <div className="text-[13px]">{site.appeal || "—"}</div>
        </div>
        <div className="border border-rule px-1.5 py-1">
          <div className="text-ink3">RIESGO</div>
      ...
      {st.locked && (
        <div className="mono mt-2 border border-spot px-2 py-1 text-[10px] uppercase text-spot">
          Sellado: todavía no tiene manera de entrar
        </div>
      )}
      {st.spoiled && (
        <div className="mono mt-2 border border-spot px-2 py-1 text-[10px] uppercase text-spot">
          Arruinado: ya no vale nada para el chárter
        </div>
      )}

      {state.race === "mantid" && st.specimens.length > 0 && (
        <div className="mono mt-2 border border-rule px-2 py-1 text-[10px]">
          <div className="text-ink3">ESPECÍMENES RETENIDOS</div>
          {st.specimens.slice(0, 4).map((sp) => {
            ...
            return (
              <div key={sp.id} className={wanted ? "text-spot" : "text-ink3"}>
                {wanted ? "▸ " : "  "}grado {sp.grade} · {sp.traits.join(" + ")}
                {wanted ? " · PEDIDO" : ""}
              </div>
            );
          })}
      ...
      {state.race === "mantid" && (
        <div className="mono mt-2 flex items-center justify-between gap-2 text-[10px]">
          <span>
            {st.isLab ? "LAB" : "sin laboratorio"} · {st.specimens.length} specimens ·{" "}
            {st.sequences.length} secuencias
          </span>
          {!st.isLab && site.kind !== "base" && (
            ...
            >
              Construir laboratorio ({LAB_COST})
            </button>
          )}
        </div>
      )}
    ...
    <div className="paper-card p-2">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="mono truncate text-[10px] text-ink3">
            {craft?.name} · {route.runs} viajes · {nightsPerLeg.toFixed(1)} noches/tramo
          </div>
        </div>
        <div className="mono shrink-0 text-right text-[10px]">
          {state.race !== "mantid" && (
            <div>
              ...
              {value.toFixed(0)}/viaje
            </div>
          )}
          <div className="text-spot">+{suspicion.toFixed(1)} ruido/tramo</div>
        </div>
      </div>
        ...
        <Dial
          ...
          options={state.fleet
            ...
            .map((c) => {
              ...
              return {
                id: c.id,
                text: `${d?.name ?? c.defId}${c.id === route.craftId ? " (en esta ruta)" : " (libre)"} — carga ${d?.capacity}, velocidad ${d?.speed}, ruido ${d?.noise}`,
              };
            })}
        />
        <Dial
          ...
          options={crewsFor(state.race).map((c) => ({
            id: c.id,
            text: `${c.name} — carga ×${c.yieldMul}, ruido ×${c.noiseMul}`,
          }))}
        />
        <Dial
          label="Corredor"
          ...
          options={CORRIDORS.map((c) => ({
            id: c.id,
            text: `${c.name} — distancia ×${c.lengthMul}, ruido ×${c.noiseMul}`,
          }))}
        />
      </div>
      ...
      <div className="mt-1.5 flex gap-1.5">
        <button className="slab px-3 py-2 text-[11px]" onClick={() => onToggle(route.id)}>
          {route.paused ? "Reanudar vuelos" : "Pausar vuelos"}
        </button>
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="mono text-[11px]">
        {state.fleet.length} cascos · {idle.length} libres · {state.routes.length}/
        {race.maxRoutes} rutas conectadas
      </div>
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="mono text-[10px] text-ink3">
                  carga {c.capacity} · velocidad {c.speed} · ruido ×{c.noise} · mantenimiento {c.upkeep}
                </div>
              </div>
              <button
...
export function Reputacións({ state }: { state: GameState }) {
  return (
